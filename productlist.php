            ...
            <?php while ($row = mysqli_fetch_assoc($result)): ?>
                <div class="col-md-4">
                    <div class="card mb-4" style="width: 18rem;">
                        <div class="card-body">
                            <h5 class="card-title"><?= htmlspecialchars($row['NamaBarang']) ?></h5>
                            <p class="card-text"><?= htmlspecialchars($row['Jumlah']) ?> <?= htmlspecialchars($row['Satuan']) ?></p>
                            <p class="card-text">Price: Rp <?= number_format($row['HargaJual'], 0, ',', '.') ?></p>
                            <form action="productlist.php" method="GET">
                                <div class="input-group mb-3">
                                    <input type="number" name="quantity" class="form-control" value="1" min="1" max="<?= $row['Jumlah'] ?>" required>
                                    <button type="submit" name="add_to_cart" class="btn btn-success" value="1">Add to Cart</button>
